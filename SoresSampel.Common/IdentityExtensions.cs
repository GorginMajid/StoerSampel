...
namespace StorSampel.Common
{
    public static class IdentityExtensions
    {
        ...
        {
            return $"{identity.GetUserFirstName()} {identity.GetUserLastName()}";
        }

        public static string GetUserDisplayName(this IIdentity identity)
        {
            var fullName = identity.GetUserFullName();
            return string.IsNullOrWhiteSpace(fullName) ? identity.GetUserName() : fullName;
        }
